...
"""
    ...
    print("Context:", out.context[:50] if out.context else None)
    print(
        "Decision Outcome:", out.decision_outcome[:50] if out.decision_outcome else None
    )
    print("Decision Drivers:", out.decision_drivers)
    print("Considered Options:", out.considered_options)
    print(
        "Decision Consequences:",
        out.decision_consequences[:50] if out.decision_consequences else None,
    )
    print("Decision Confirmation:", out.decision_confirmation)
    print(
        "Options Pros/Cons:",
        {k: len(v) for k, v in (out.options_pros_and_cons or {}).items()},
    )
    print("More Info:", out.more_info[:50] if out.more_info else None)
